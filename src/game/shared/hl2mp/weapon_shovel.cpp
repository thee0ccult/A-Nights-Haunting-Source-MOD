...
//-----------------------------------------------------------------------------
ConVar sk_shovel_lead_time( "sk_shovel_lead_time", "0.9" );

int CWeaponShovel::WeaponMeleeAttack1Condition( float flDot, float flDist )
{
	// Attempt to lead the target (needed because citizens can't hit manhacks with the crowbar!)
	...
	float dt = sk_shovel_lead_time.GetFloat();
...
float CWeaponShovel::GetRange( void )
{
	return	CROWBAR_RANGE;	
}

float CWeaponShovel::GetFireRate( void )
{
